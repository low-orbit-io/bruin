...
Bruin is a lightweight state manager, particularly used with React. Bruin avoids reducers, context, and boilerplate.
...
Bruin provides a built-in helper called `ExtractState`. This is useful for tests, utility functions, or component props.
...
```ts
// store.ts
import { create, type ExtractState } from '@inboxhealth/bruin';
...
Bruin together with TypeScript provides a balance: you keep the simplicity of small, minimalistic stores, while gaining the safety of strong typing.
