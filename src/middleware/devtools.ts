...
import type { NamedSet as NamedSetType, Write } from '../types/middleware';
...
const devtoolsImpl =
  ...
  (set, get, api) => {
    ...
    if (!enabled) {
      return runInitializer(set as unknown as NamedSetType<S>, get as any, api);
    }
    ...
    if (storeId && multiStore) {
      ...
      try {
        ...
        return runInitializer(
          set as unknown as NamedSetType<S>,
          get as any,
          api,
        );
      }
    ...
    (api as unknown as ApiType).setState = (
      ...
      if (typeof actionName === 'string') {
        ...
        const anonymousType =
          anonymousActionType || (storeId ? '...' : 'anonymous');
    ...
    const initialState = runInitializer(setStateWithDevtools, get as any, api);
