...
Although Bruin is an unopinionated library, we do recommend a few patterns.
...
However, Bruin does differ in some fundamental ways,
...
Your applications global state should be located in a single Bruin store.

If you have a large application, Bruin supports [splitting the store into slices](./slices-pattern.md).
...
In Bruin, state can be updated without the use of dispatched actions and reducers found in other Flux libraries.
